...
pof_domicilio <- read_rds("/Users/apple/Documents/TCC/dados_rds/pof_domicilio.rds") # nolint
pof_morador <- read_rds("/Users/apple/Documents/TCC/dados_rds/pof_morador.rds") # nolint
...
pof_domicilio <- pof_domicilio %>%
    select(V6199, NUM_DOM, id_dom) ##V6199 - Insegurança Alimentar

pof_morador %>%
    filter(V0403 <=19) %>% ## V0403 - Idade
    filter(V0306 != 01 & 17 & 18 & 19 ) %>% ## V0306 - Condição na UC - filho, neto e etc... # nolint
    select(UF, ESTRATO_POF, PESO, PESO_FINAL,V0403,V0306, INSTRUCAO, COMPOSICAO,
     PC_RENDA_DISP, PC_RENDA_MONET, PC_RENDA_NAO_MONET, id_dom, id_uc, id_pes) %>% glimpse()
